...
#######################################################################################################################################
df_titanic <- fread('../../data/titanic.csv', data.table = FALSE)
classification_dataset <- df_titanic %>%
    dplyr::select(-PassengerId, -Name, -Ticket, -Cabin) %>%
    dplyr::mutate(Survived = ifelse(Survived == 1, 'lived', 'died')) %>%
    dplyr::rename(target = Survived)

classification_dataset <- as.data.frame(classification_dataset)

target_positive_class <- 'lived'
target_negative_class <- 'died'
```
...
skewnewss_statistics <- map_dbl(classification_column_names, ~ {
    ...
    if(is.numeric(dataset_column) && !any(is.na(dataset_column)) && min(dataset_column > 0)) {
...
df_outliers <- map(numeric_column_names, ~ {
    ...
    lower_quantile <- quantile(numeric_data, na.rm = TRUE)[2]
    upper_quantile <- quantile(numeric_data, na.rm = TRUE)[4]
    ...
    upper_outlier_count <- sum(numeric_data > threshold_upper, na.rm = TRUE)
    lower_outlier_count <- sum(numeric_data < threshold_lower, na.rm = TRUE)
...
if(sum(numeric_columns == TRUE) >= 2) {
    ...
    matrix_correlations <- cor(classification_dataset[, numeric_columns][complete.cases(classification_dataset[, numeric_columns]), ])
...
walk(classification_column_names, ~ {
    ...
    if(is.numeric(dataset_column)) {
        ...
        fun_args <- list(mean = mean(df_data$target), sd = sd(df_data$target, na.rm=TRUE))
        quantiles <- quantile(df_data$target, c(0.02, 0.98), na.rm=TRUE) # cut off any 'outliers' (crude) so it doesn't skew graph
    ...
    if(refresh_models) {
        ...
        if(!is.null(neural_net_numWts) || !is.null(neural_net_max_iterations) || !is.null(neural_net_lineout)) { # neural network
            ...
            caret_model <- train(   form = target ~ .,
                                    ...
                                    preProcess = training_preProcess,
...
```{r impute, echo = FALSE, comment = '', results = 'asis'}
# caret isn't fucking imputing values.. 
imputation <- preProcess(classification_train, method = 'medianImpute')

classification_train <- predict(imputation, classification_train)
classification_test <- predict(imputation, classification_test)
```
## Models
...
#asdf <- preProcess(classification_train %>% dplyr::select(Fare, Age, target), method = 'medianImpute')
#all(complete.cases(predict(asdf, classification_train %>% dplyr::select(Fare, Age, target))))
glm_no_pre_process <- train_classification(
    ...
    training_method = 'glm',
    training_preProcess = "medianImpute",
