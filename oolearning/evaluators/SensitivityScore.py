...
class SensitivityScore(SupportsTwoClassClassificationMixin, UtilityFunctionMixin, ClassificationScoreBase):
    @property
    def name(self) -> str:
    ...
    def _calculate(self, actual_values: np.ndarray, predicted_values: np.ndarray) -> float:
        return TwoClassEvaluator.from_classes(actual_classes=actual_values,
                                              ...
                                              positive_category=self._positive_class).true_positive_rate
